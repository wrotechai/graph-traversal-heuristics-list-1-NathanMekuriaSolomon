...
def _cost_of_tour(depot_ids, ordered_ids, graph, stops, start_time, criterion):
    # Evaluate complete tour.
    # Return arrival, paths.
    full_path = []
    current_ids  = list(depot_ids)
    current_time = start_time

    legs = list(ordered_ids) + [None]   # None signals "return to depot"
    for i, target_id in enumerate(legs):
...
def _greedy_init(depot_ids, visit_ids, graph, stops, start_time, criterion):
    ...
    while remaining:
        ...
        if best_target is None:
            break  # unreachable stops remain — skip them
...
def tabu_search(depot_ids, visit_ids, graph, stops, start_time, criterion,
    ...
    if n == 1:
        cost, path = _cost_of_tour(depot_ids, visit_ids, graph, stops,
    ...
    # Tabu search
    # Neighbourhood: all 2-opt swaps (reverse a sub-sequence)
    tabu_list: dict = {}   # swap_key -> iteration it expires
    ...
    for iteration in range(max_iter):
        ...
        for i in range(n - 1):
            for j in range(i + 1, n):
                ...
                # Generate neighbour by reversing segment [i..j]
        ...
        if best_neighbour_order is None:
            break   # no improving or non-tabu move found
